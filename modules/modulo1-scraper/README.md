...
Cuenta con un flujo real operativo (API REST + Gemini): Descarga incremental de convocatorias desde la API de BDNS y análisis semántico profundo empleando modelos fundacionales de Google Gemini.
...
│   └── vectorizer.py                 # Vectorizador local SentenceTransformer (E5) para guardado atómico
├── tests/
│   ├── __init__.py
...
├── requirements.txt                  # Dependencias del módulo actualizadas (bdns-fetch, google-generativeai, sentence-transformers)
...
Asegúrate de disponer de tu clave de API en el archivo `.env` en la raíz del proyecto o en `modules/modulo2-db/.env`:
...
El flujo de recogida y procesamiento semántico se divide en dos fases independientes:
...
Para garantizar que el scraper funciona correctamente, el proyecto incorpora suites de pruebas con `pytest`.
...
## Contrato de Datos (Convocatorias Enriquecidas)
